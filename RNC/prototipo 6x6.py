...
for i in range(0,len(attack2)):
  if i < 8956:
    ...
    titulo = ('./imagens/train/attack/attack'+(str(i+1))+'.png')
    im.save(titulo)
    ...
    #  info.write(titulo2+"\n")
    #  info.read()
  elif i >= 8956 and i < 17912:
    attack3 = [attack2[i]]
    attack4 =np.resize(attack3,(6,6))
    attack4 = np.array(attack4,np.uint8)
    im = Image.fromarray((attack4))
    titulo = ('./imagens/validation/attack'+(str(i+1))+'.png')
    im.save(titulo)
  else:
    attack3 = [attack2[i]]
    attack4 =np.resize(attack3,(6,6))
    attack4 = np.array(attack4,np.uint8)
    im = Image.fromarray((attack4))
    titulo = ('./imagens/test/attack'+(str(i+1))+'.png')
    im.save(titulo)
...
for i in range(0,len(normal2)):
  if i < 9294:
    ...
    titulo = ('./imagens/train/normal/normal'+(str(i+1))+'.png')
    im.save(titulo)
    #with open("./normal6x6/normal.txt","a+") as bg:
    #  bg.write(titulo2+"\n")
    #  bg.read()
  elif i >= 9294 and i < 18588:
    normal3 = [normal2[i]]
    normal4 =np.resize(normal3,(6,6))
    normal4 = np.array(normal4,np.uint8)
    im = Image.fromarray((normal4))
    titulo = ('./imagens/validation/normal'+(str(i+1))+'.png')
    im.save(titulo)
  else:
    normal3 = [normal2[i]]
    normal4 =np.resize(normal3,(6,6))
    normal4 = np.array(normal4,np.uint8)
    im = Image.fromarray((normal4))
    titulo = ('./imagens/test/normal'+(str(i+1))+'.png')
    im.save(titulo)